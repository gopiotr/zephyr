...
class Shell:
    ...
    def exec_command(self, command: str, timeout: float | None = None) -> list[str]:
        """
        ...
        lines: list[str] = self._device.readlines_until(regex=re.escape(self.prompt), timeout=timeout)
        lines.extend(self._device.readlines_until(regex=re.escape(self.prompt), timeout=timeout))
        return lines
