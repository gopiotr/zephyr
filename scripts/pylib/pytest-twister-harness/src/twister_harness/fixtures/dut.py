...
@pytest.fixture(scope='session')
def device_object(
    request: pytest.FixtureRequest, twister_harness_config: TwisterHarnessConfig
    ...
    device_object = device_class(device_config, test_name=test_name)
    try:
        yield device_object
...
def dut(device_object: DeviceAdapter) -> Generator[DeviceAdapter, None, None]:
